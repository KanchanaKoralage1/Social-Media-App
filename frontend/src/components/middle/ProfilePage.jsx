...
const ProfilePage = () => {
  ...
  const isCurrentUserProfile = currentLoggedInUser && profile && currentLoggedInUser.username === profile.username

  useEffect(() => {
    const fetchProfileAndPosts = async () => {
      const token = localStorage.getItem("token")
      // Highlight: Use username from URL if present, otherwise fetch current user's profile
      const url = username ? `http://localhost:8080/api/profile/${username}` : "http://localhost:8080/api/profile"
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (currentLoggedInUser && currentLoggedInUser.username === updated.username) {
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/users/${profile.username}/${action}`, {
        method: method,
        headers: {
      ...
      if (res.ok) {
        ...
        setProfile((prevProfile) => ({
          ...prevProfile,
          followersCount: isFollowing ? prevProfile.followersCount - 1 : prevProfile.followersCount + 1,
        }))
      } else {
        const errorText = await res.text()
  ...
  const handleEdit = async (post, updatedContent, keptImages, newImageFiles) => {
    ...
    if (res.ok) {
      ...
      const postsRes = await fetch(`http://localhost:8080/api/posts/user/${profile.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      const postsData = await postsRes.json()
      setPosts(
    ...
    <div className="max-w-2xl mx-auto mt-8 bg-gray-100 rounded shadow overflow-hidden">
      ...
      {editMode && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <form onSubmit={handleUpdate} className="bg-white rounded shadow-lg p-6 w-full max-w-md relative">
            <button className="absolute top-2 right-2 text-gray-500" type="button" onClick={() => setEditMode(false)}>
              ✕
            </button>
      ...
      <div className="mt-8 px-6 pb-6">
        <h2 className="text-xl font-bold mb-4">Posts</h2> {/* Changed "My Posts" to "Posts" */}
        {posts.length === 0 ? (
